 ...
 */
class DatastoreClient extends EventEmitter {
    ...
    /** 
     * @event DatastoreClient#synchronization
     * @property {boolean} old_value - previous synchronization state
     * @property {boolean} new_value - new synchronization state
     */
    ...
    async _runloop() {
        ...
        try {
            if (this.#state === STATE.INITIAL) {
                ...
                this.#log.debug(`fetched ${all_items.length} items: ${JSON.stringify(all_items)}`);
                ...
                this._set_synchronization(true);
                this.#backoff.reset();
            ...
            this.#log.error(`Error in runloop: ${error.message}`);

            // If we are not in INITIAL state, trigger a resync.  Otherwise, just backoff
            if (this.#state != STATE.INITIAL) {
                
                this.#log.info('Resync triggered');
                this.#log.debug(`Current data set: ${JSON.stringify(this.#synchroset.all().map(item => item.toObject()))}`);
                this._set_synchronization(false);
     ...
     * @emit DatastoreClient#synchronization
     */
    _set_synchronization(new_value) {

        let old_value = this.#synchronized;
        this.#synchronized = new_value;

        if (old_value !== new_value) { this.emit('synchronization', old_value, new_value); }
    }
     ...
     * @throws {Error} if the updates cannot be processed
     */
    async _process_updates(updates) {
        ...
        if (metadata) {
            ...
            if (metadata.start_checksum && metadata.start_checksum !== currentChecksum) {
                throw new Error(`checksum mismatch in update: expected start checksum ${metadata.start_checksum}, but our current checksum is ${currentChecksum}.`);
            }
        }
        ...
        if (metadata && metadata.end_checksum) {
            ...
            if (finalChecksum !== metadata.end_checksum) {
                throw new Error(`checksum mismatch after applying updates: expected final checksum to be ${metadata.end_checksum}, but current checksum is ${finalChecksum}`);
            } else {
    ...
    async start() {
        ...
        this._set_synchronization(false);  // 'Cos how would we know?  We were off
    ...
    stop() {
        ...
        this.#backoff.reset();
        this._set_synchronization(false); // 'Cos how would we know?  We're off
    }
    ...
    get synchronized() { return this.#synchronized; }
