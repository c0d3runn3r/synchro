...
class Notion extends EventEmitter {
    ...
    static fromObject(obj) {
        ...
        const notion = new Notion(obj.name);
        if( obj.value) {

            if(obj.timestamp) {
                notion.set_value(obj.value, obj.timestamp);
            } else {
                notion.value = obj.value;
            }
        }
        return notion;
    }
    ...
    set_value(value, timestamp) {
        ...
        // Only emit a change event if the value or timestamp has actually changed
        if( old_value === this.#value && old_timestamp?.getTime() === this.#timestamp?.getTime()) { return; }
