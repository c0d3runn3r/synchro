...
class Pulsar {
    ...
    constructor(opts={ interval: 0, collapse: true, transmit: null, allow_empty: true, include_checksums: true }) {
        ...
        this.#include_checksums = opts.include_checksums ?? true; // Default to including checksums
    ...
    /**
     * Get whether checksums should be included in transmissions.
     * 
     * @returns {boolean} true if checksums should be included, false otherwise
     */
    get include_checksums() { return this.#include_checksums; }

    /**
     * Set whether checksums should be included in transmissions.
     * 
     * @param {boolean} value true to include checksums, false to exclude them
     */
    set include_checksums(value) { this.#include_checksums = !!value; }
    ...
    trigger() {
        ...
        // Collapse the queue to an array of non null payloads
        ...
        // If include_checksums is enabled, prepend checksum metadata to the bundle
        if (this.#include_checksums && (this.#start_checksum || this.#end_checksum)) {
            const metadata = JSON.stringify({
                event_name: 'comment',
                _metadata: true,
                start_checksum: this.#start_checksum,
                end_checksum: this.#end_checksum
            });
            bundle.unshift(metadata);
        }
     ...
     * @param {string} old_checksum the expected checksum of the set before the change has been applied
     * @param {string} checksum the expected checksum of the set after the change has been applied
     */
    queue(event_name, item, change, old_checksum, checksum) {

        // Track checksums as required
        // Only set start_checksum if this is the first event in the queue
        if (this.#include_checksums && this.#queue.length === 0 && old_checksum) { 
            this.#start_checksum = old_checksum; 
        }
        if (this.#include_checksums && checksum) { this.#end_checksum = checksum; }
    ...
    empty() {
        ...
        this.#added = {};
        this.#changes_by_id = {};
        this.#start_checksum = null;
        this.#end_checksum = null;
    }
