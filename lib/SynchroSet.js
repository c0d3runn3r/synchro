...
class SynchroSet extends EventEmitter {
    ...
    /**
     * Coerce the set to match a given array of items, causing relevant events to be emitted
     * 
     * This is useful for forcefully synchronizing the set with a known state
     * 
     * @param {Class[]} items an array of items to coerce to
    ...
    coerce_set_to(items = []) {
        ...
        existing.forEach(item => this.coerce_to(item));
