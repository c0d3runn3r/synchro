...
/**
 * @event SynchroSet#receive_checksum_mismatch
 * @property {string} expected - The checksum we were told to expect after applying the received changes
 * @property {string} actual   - The actual checksum of the set after applying the received changes
 * @description When we receive a change set, we expect to apply those changes and have our checksums match. If they do not, this event is emitted.
 */
...
class SynchroSet extends EventEmitter {
     ...
     *
     * @param {function({string} payload, {string} checksum)|[function({string} payload, {string} checksum)]|Pulsar|[Pulsar]} fn function or array of functions or Pulsars that will transmit our event payloads. Set to null to disable transmission.
    ...
    _transmit(event_name, item, event) {
        ...
        for (const fn of this.#transmit) {
            ...
            const payload = SynchroSet.payload(event_name, item, change, this.checksum);
            if(fn?.constructor?.name === 'Pulsar') { fn.queue(event_name, item, change, this.checksum); continue; }
            fn(payload, this.checksum);
        }
    }
    ...
    /**
     * Handle incoming payload strings, parsing them and updating the set accordingly
     * @param {string} str
     * @param [string] expected_checksum - if provided, will be comared with our checksum and an event emitted if they do not match
     * @emits SynchroSet#receive_checksum_mismatch
     */
    receive(str, expected_checksum) {
        ...
        // If we have an expected checksum, compare it with our current checksum
        if (expected_checksum) {
            const actual_checksum = this.checksum;
            if (actual_checksum !== expected_checksum) {
                this.emit('receive_checksum_mismatch', { expected: expected_checksum, actual: actual_checksum });
            }
        }
