...
class SynchroSet extends EventEmitter {
    ...
    constructor(my_class = SynchroItem) {
        ...
        this.#old_checksum = this.checksum; // Initialize old_checksum to current checksum (empty set checksum)
    }
    ...
    _transmit(event_name, item, event) {
        ...
        if (this.#transmit) {
            ...
            for (const fn of this.#transmit) {
                ...
                if (fn?.constructor?.name === 'Pulsar') { 
                    fn.queue(event_name, item, change, this.#old_checksum, this.checksum); 
                    continue; 
                }
        ...
        this.#old_checksum = this.checksum; // Store the checksum at the time of transmission
    }
    ...
    update_set_to(items = []) {
        ...
        for (let item of items) {
            ...
            if (this.find(item.id)) { existing.push(item); } else { add.push(item); }
        }
        ...
        existing.forEach(item => this.find(item.id).update_to(item));
    ...
    get checksum() {
        if (this.#checksum === null) {
            ...
            // Recalculate checksum.  Start by sorting items by ID for consistent ordering
            const sorted_items = this.all().sort((a, b) => a.id.localeCompare(b.id));
            this.#checksum = crypto.createHash('sha256').update(sorted_items.map(item => item.checksum).join(''), 'utf8').digest('hex');
        }
