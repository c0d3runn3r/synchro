...
class SynchroSet extends EventEmitter {
     ...
     * 
     * @param {function} fn function that takes an event name and payload to transmit. Set to null to disable transmission.
     * @throws {TypeError} if the function is not a function
     */
    set transmit(fn) {
        if (typeof fn !== 'function') { throw new TypeError('Expected a function to transmit changes'); }
        this.#transmit = fn;
    }
    ...
    _transmit(event_name, item, event) {
        ...
        switch(event_name) {
            case 'added':
                this.#transmit(JSON.stringify({ event_name, item: item.toObject() }));
                break;
            case 'removed':
                this.#transmit(JSON.stringify({ event_name,  item: { id:item.id } }));
                break;
            case 'changed':
                this.#transmit(JSON.stringify({ event_name, change: cleaned_event }));
                break;
        }
    ...
    add(item) {
        ...
        item.on('changed', (event) => {
            this.emit('changed', { item, event });
            this._transmit('changed', item, event );
        });
     ...
     * @param {Class|string} item the item to remove, or its ID 
     */
    remove(item) {
        ...
        if (typeof item === 'string') {
            item = this.find(item);
            if (!item) { throw new Error(`Item with ID ${item} not found`); }

        } else if (!(item instanceof this.#my_class)) {

            throw new TypeError('Expected an instance of the managed class or a string ID');
        }
